...
import { UserProfile, InterviewTopic, TopicStatus } from './types';
import { generateInterviewTopics, analyzeInterviewVideo } from './services/geminiService';
import { Layout, Video, ChevronRight, CheckCircle2, Loader2, PlayCircle, Plus, RefreshCw, FileText, X } from 'lucide-react';

const App: React.FC = () => {
  const [step, setStep] = useState<'SETUP' | 'TOPIC_SELECTION' | 'RECORDING' | 'REVIEW'>('SETUP');
  const [profile, setProfile] = useState<UserProfile | null>(null);
  const [topics, setTopics] = useState<InterviewTopic[]>([]);
  const [currentTopicId, setCurrentTopicId] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [isRegenerating, setIsRegenerating] = useState(false); // Explicit state for regeneration
  ...
  // Script Editor State
  const [editingScriptTopicId, setEditingScriptTopicId] = useState<string | null>(null);
  ...
  const handleSetupComplete = async (userProfile: UserProfile) => {
    ...
    try {
      ...
      alert("生成选题失败，请检查 API Key 是否配置正确。");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegenerateTopics = async () => {
    if (!profile) return;
    if (!window.confirm("确定要换一批新的选题吗？当前未录制的选题将被替换。")) return;

    setIsRegenerating(true); // Start loading UI immediately
    setLoading(true);
    setTopics([]); // Clear existing to prevent confusion
  ...
  const handleRecordingComplete = async (blob: Blob) => {
    ...
    // Update local state first to show we have the video
    setTopics(prev => prev.map(t => t.id === currentTopicId ? { ...t, videoBlob: blob, status: TopicStatus.ANALYZING } : t));
    setProcessingVideo(true);

    try {
      const currentTopic = topics.find(t => t.id === currentTopicId);
      if (currentTopic) {
        const analysis = await analyzeInterviewVideo(blob, currentTopic.question);
      ...
      alert("视频分析失败。");
      setTopics(prev => prev.map(t => t.id === currentTopicId ? { ...t, status: TopicStatus.RECORDED } : t)); // Revert status
    } finally {
  ...
  const handleNext = () => {
    // Check if there are pending topics
    ...
    if (hasPending) {
      ...
      // All done
      alert("所有选题已完成！");
      setStep('TOPIC_SELECTION'); // Or go to a summary page
    }
  };
      ...
      <header className="border-b border-gray-800 bg-gray-950/50 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-xl font-bold tracking-tight">AI 商业 IP 编导 <span className="text-indigo-500 text-sm font-normal bg-indigo-500/10 px-2 py-0.5 rounded-full ml-2">Beta</span></h1>
          </div>
          {profile && (
      ...
      <main className="flex-1 flex flex-col">
          ...
          <div className="max-w-5xl mx-auto w-full p-6 space-y-8 relative">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="space-y-2">
                    ...
                    <p className="text-gray-400">选择一个选题开始录制，或添加自定义选题。</p>
                </div>
                <div className="flex space-x-3">
                     ...
                     >
                        {(loading || isRegenerating) ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                        <span>换一批</span>
                     </button>
                </div>
            </div>
            ...
            {showCustomTopicInput && (
                <div className="bg-gray-900 border border-indigo-500/50 p-4 rounded-xl animate-fade-in">
                    <label className="block text-sm font-medium text-gray-300 mb-2">输入自定义选题</label>
            ...
            <div className="grid gap-6">
              {(loading || isRegenerating) ? (
                 <div className="flex flex-col items-center justify-center py-20 bg-gray-900/50 rounded-2xl border border-gray-800 border-dashed animate-pulse">
                     ...
                     <p className="text-gray-400">AI 正在为您策划新的爆款选题...</p>
                 </div>
              ) : (
                  topics.map((topic, index) => (
                    <div 
                    key={topic.id} 
                    className={`relative bg-gray-900 border ${topic.status === TopicStatus.COMPLETED ? 'border-green-900' : 'border-gray-800'} rounded-2xl p-6 transition-all hover:border-gray-600 group`}
                    >
                    <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                        ...
                        {topic.script && (
                             <div className="mt-2 text-xs text-gray-400 bg-gray-800/50 p-2 rounded inline-block">
                                <span className="font-semibold text-indigo-400">已添加提词: </span>
                                {topic.script.length > 20 ? topic.script.substring(0, 20) + '...' : topic.script}
                             </div>
                        )}
                        </div>
                        ...
                        <div className="flex items-center space-x-3">
                         ...
                         {topic.status !== TopicStatus.COMPLETED && (
                             <button
                                ...
                                className={`px-4 py-3 rounded-lg font-medium transition flex items-center ${topic.script ? 'bg-indigo-900/30 text-indigo-300 hover:bg-indigo-900/50' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                                title="添加/编辑提词器文案"
                             >
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-gray-900 border border-gray-700 rounded-xl max-w-2xl w-full shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center p-6 border-b border-gray-800">
                            ...
                            <button onClick={() => setEditingScriptTopicId(null)} className="text-gray-400 hover:text-white">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        <div className="p-6 flex-1 overflow-y-auto">
                            <p className="text-sm text-gray-400 mb-2">为该选题输入您的回答大纲或完整文案，录制时将作为提词器显示。</p>
                            <textarea
                                value={tempScript}
        ...
        {processingVideo && (
           <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center text-center p-6">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">AI 编导正在复盘中...</h2>
        ...
        {step === 'REVIEW' && activeTopic && activeTopic.analysis && activeTopic.videoBlob && (
           <AnalysisView 
              result={activeTopic.analysis}
              videoBlob={activeTopic.videoBlob}
              onNext={handleNext}
              isLastTopic={topics.every(t => t.status === TopicStatus.COMPLETED)}
           />
        )}
