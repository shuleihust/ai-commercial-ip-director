...
import { Download, Share2, Star, TrendingUp, Users, AlertCircle } from 'lucide-react';
...
export const AnalysisView: React.FC<AnalysisViewProps> = ({ result, videoBlob, onNext, isLastTopic }) => {
      ...
      <div className="space-y-6 overflow-y-auto max-h-[calc(100vh-100px)] custom-scrollbar">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-white">{result.score.total}</div>
                <div className="text-xs text-gray-400 uppercase tracking-wide mt-1">综合得分</div>
            </div>
            <div className="bg-gray-800 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-indigo-400">{result.score.traffic}</div>
            ...
            <div className="bg-gray-800 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-green-400">{result.score.leads}</div>
                <div className="text-xs text-gray-400 uppercase tracking-wide mt-1 flex justify-center items-center gap-1">
        ...
        <div className="bg-indigo-900/20 border border-indigo-500/30 rounded-xl p-6">
           <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-indigo-100">爆款短视频结构重组</h3>
              <span className="text-xs bg-indigo-500 text-white px-2 py-1 rounded">原话重组</span>
           </div>
           ...
           <div className="space-y-4 text-sm">
              <div className="relative pl-4 border-l-2 border-pink-500">
                  ...
                  <p className="text-pink-400 font-bold text-xs uppercase mb-1">黄金开头 (Hook)</p>
                  <p className="text-gray-200">{result.viralStructure.hook || <span className="text-gray-500 italic">未检测到强有力的 Hook。</span>}</p>
              </div>
              
              <div className="relative pl-4 border-l-2 border-gray-600">
                  <p className="text-gray-400 font-bold text-xs uppercase mb-1">痛点 / 背景 (Pain)</p>
                  <p className="text-gray-300">{result.viralStructure.painPoint}</p>
              </div>

              <div className="relative pl-4 border-l-2 border-gray-600">
                  <p className="text-gray-400 font-bold text-xs uppercase mb-1">解决方案 / 价值 (Solution)</p>
              ...
              <div className="relative pl-4 border-l-2 border-green-500">
                  ...
                  <p className="text-green-400 font-bold text-xs uppercase mb-1">行动号召 (CTA)</p>
                  <p className="text-gray-200">{result.viralStructure.cta || <span className="text-gray-500 italic">未检测到明显的行动号召。</span>}</p>
              </div>
           </div>
        </div>
