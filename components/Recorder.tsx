...
import { Video, Mic, StopCircle, RefreshCw, CheckCircle, Smartphone, Monitor, Volume2, FileText, X, Settings2, Square } from 'lucide-react';
...
export const Recorder: React.FC<RecorderProps> = ({ question, script, onRecordingComplete, onCancel }) => {
  ...
  const [aspectRatio, setAspectRatio] = useState<AspectRatio>('portrait'); // Default to portrait for TikTok/Shorts
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      // Stream cleanup handled in unmount effect
    };
  ...
  const startRecording = useCallback(() => {
    ...
    stopAudio(); // Ensure TTS stops when recording starts
    setRecordedChunks([]);
  ...
  // Watch chunks to trigger complete
  const handleFinish = () => {
  ...
  const playAudio = async () => {
      ...
      try {
          ...
          // Map user selection to Gemini Voice Names
          // Zephyr: Soft/Gentle (Mapped to Taiwanese style)
          // Fenrir: Deep/Intense (Mapped to Broadcast Male)
          // Kore: Calm (Mapped to Female Teacher)
          ...
          const audioBuffer = audioContextRef.current.createBuffer(channelCount, frameCount, sampleRate);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
            ...
            <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-2 mt-2 bg-black/40 backdrop-blur-md rounded-full px-3 py-1 border border-white/10">
                     ...
                     <button 
                        ...
                        className={`p-1 rounded-full transition flex-shrink-0 ${isPlayingAudio ? 'text-red-400 hover:text-red-300' : 'text-indigo-400 hover:text-indigo-300'}`}
        ...
        <div className="absolute inset-0 z-20 pointer-events-none flex flex-col items-center pt-40 px-4 pb-32">
            <div className="bg-black/40 backdrop-blur-sm p-6 rounded-xl max-w-2xl w-full max-h-full overflow-y-auto pointer-events-auto text-center border border-white/10 shadow-2xl">
                <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
                    <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">提词器 (Teleprompter)</span>
                    <button onClick={() => setShowTeleprompter(false)} className="text-gray-400 hover:text-white">
                        <X className="w-4 h-4" />
                    </button>
                </div>
      ...
      <div className="flex-1 relative bg-gray-900 overflow-hidden flex items-center justify-center">
        {/* Container to enforce aspect ratio visualization if needed, but video object-cover usually handles it */}
        <video
          ref={videoRef}
          autoPlay
          muted
          playsInline
          className={`object-cover transform scale-x-[-1] transition-all duration-500 ${aspectRatio === 'portrait' ? 'h-full w-auto aspect-[9/16]' : 'w-full h-auto aspect-[16/9]'}`}
        />
        
        {/* Aspect Ratio Toggle (Only show when not recording) */}
        {!isRecording && recordedChunks.length === 0 && (
            <div className="absolute bottom-12 right-6 z-20 flex flex-col space-y-2 bg-black/50 p-2 rounded-lg backdrop-blur-sm">
                <button
                    ...
                    className={`p-2 rounded-md transition ${aspectRatio === 'portrait' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
                    title="竖屏 (9:16)"
                >
                ...
                <button
                    ...
                    className={`p-2 rounded-md transition ${aspectRatio === 'landscape' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
                    title="横屏 (16:9)"
                >
                    <Monitor className="w-6 h-6" />
                </button>
                {/* Teleprompter Toggle for Pre-recording setup */}
                {script && (
                    <button
                        onClick={() => setShowTeleprompter(!showTeleprompter)}
                        className={`p-2 rounded-md transition ${showTeleprompter ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
                        title="打开/关闭提词器"
                    >
        ...
        {isRecording && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none">
             {/* Optional grid or guides could go here */}
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-950 p-6 border-t border-gray-800 relative z-50">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
            ...
            {/* Right Action (Spacer or Settings) */}
            <div className="w-[60px] flex justify-end">
                {/* Script Toggle accessible during recording setup */}
                {script && isRecording && (
                    <button
                        onClick={() => setShowTeleprompter(!showTeleprompter)}
                        className={`p-2 rounded-full bg-gray-800 transition ${showTeleprompter ? 'text-indigo-400' : 'text-gray-400'}`}
                    >
                        <FileText className="w-6 h-6" />
                    </button>
                )}
